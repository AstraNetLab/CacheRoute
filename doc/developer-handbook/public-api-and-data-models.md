...
- **Artifact fields:** computed `artifact_id`; required `knowledge_id`, `model_profile`, `tokenizer_profile`, `cache_data_profile`, `compatibility_profile_id`; defaults `artifact_version="1"`, `adapters=()`, `runtime_profile=V1`, `schema_version="v1"`, UTC `created_at`. Adapters must be non-empty and unique; tuple order is preserved as part of canonical identity and is not sorted or reordered. ID is deterministic `artifact_` plus 32 lowercase hex characters over the serialized identity.
