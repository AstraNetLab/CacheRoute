"""Start a CacheRoute Instance demo with optional resource monitoring.

This script owns demo-only Resource Agent lifecycle management, reporting,
and cleanup while keeping the core Instance request path unchanged.
"""
import uvicorn
import argparse
import os
...
def main():
    ...
    # Defaults come from config/env; command-line arguments override them.
    ...
    # Keep the listen port equal to the registration/reporting port.
    ...
    # Strongly prefer a unique instance id to avoid pool upsert overwrites.
