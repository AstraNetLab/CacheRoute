...
def resolve_scan_match(profile: str, requested_match: Optional[str]) -> str:
    ...
    if profile == RUNTIME_PROFILE_TEST_MOCK:
        raise ValueError("test/mock runtime profile cannot access real Redis key layouts")
...
def filter_supported_keys(
    ...
    profile = normalize_runtime_profile(profile)
    if profile == RUNTIME_PROFILE_TEST_MOCK:
        raise ValueError("test/mock runtime profile cannot select real Redis keys")
