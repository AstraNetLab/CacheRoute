...
class QueueManager:
    ...
    async def _recompute_pending_from_now(self, instance_id: str, now_s: Optional[float] = None) -> None:
        ...
        async with lock:
            ...
            pending = [t for t in self._instance_pending_tasks[instance_id] if not t.has_finished_forward]
            pending.sort(key=lambda t: t.reservation_seq)
            active_started = [t for t in pending if t.has_started_forward and not t.has_finished_forward]
            not_started = [t for t in pending if (not t.has_started_forward) and (not t.has_finished_forward)]
    ...
    async def _ready_worker_loop(self, instance_id: str, worker_idx: int) -> None:
        ...
        while True:
            ...
            try:
                ...
                await self._mark_task_forward_end_and_recompute(task, instance_id)
