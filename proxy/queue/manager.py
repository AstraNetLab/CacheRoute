...
class QueueManager:
    ...
    async def _apply_idle_correction(self, task: ProxyTask, instance_id: str) -> None:
        """
        任务首 token 到达后，按“预测计算时延 vs 实际计算时延”纠正 expected_idle 时间戳。

        规则：
        - actual_compute > predict_compute  -> expected_idle 后移
        - actual_compute < predict_compute  -> expected_idle 前移
        """
        ...
        logger.info(
            "[Predict-Correct] rid=%s instance=%s correction=%.2fms idle(old->new)=%.2f->%.2f ms",
            task.request_id,
            instance_id,
            correction_s * 1000.0,
            old_idle_s * 1000.0,
